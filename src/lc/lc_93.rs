...
pub struct Solution;
impl Solution {
    fn restore(s: &[u8], ip: &mut Vec<char>, split: i32, last: u8, lastn: i32, res: &mut Vec<String>) {
        if s.is_empty() {
            if split == 3 {
        ...
        if lastn != 0 && (lastn < 25 || (lastn == 25 && s[0] <= b'5')) {
            ip.push(s[0] as char);
            Self::restore(&s[1..], ip, split, s[0], lastn * 10 + (s[0] - b'0') as i32, res);
            ip.pop();
        }
        if split == 3 {
            return;
        ...
        ip.push('.');
        ip.push(s[0] as char);
        Self::restore(&s[1..], ip, split + 1, s[0], (s[0] - b'0') as i32, res);
        ip.pop();
        ip.pop();
    }
