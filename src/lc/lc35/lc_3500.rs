...
// 3500. Minimum Cost to Divide an Array
pub struct Solution;
impl Solution {
    pub fn minimum_cost(nums: Vec<i32>, cost: Vec<i32>, k: i32) -> i64 {
        let len = nums.len();
        let k = k as i64;
        let mut sum_nums = Vec::with_capacity(len + 1);
        sum_nums.push(0);
        let mut sum_cost = Vec::with_capacity(len + 1);
        sum_cost.push(0);
        for i in 0..len {
            sum_nums.push(sum_nums[i] + nums[i] as i64);
            sum_cost.push(sum_cost[i] + cost[i] as i64);
        }
        let mut c = vec![i64::MAX; len + 1];
        c[0] = 0;
        for i in 0..len {
            for j in 0..=i {
                c[i + 1] = c[i + 1]
                    .min(c[j] + sum_nums[i + 1] * (sum_cost[i + 1] - sum_cost[j]) + k * (sum_cost[len] - sum_cost[j]));
            }
        }
        c[len]
    }
